...
use linera_base::vm::{VmRuntime, EvmOperation, EvmQuery};
use linera_sdk::{
    abis::evm::EvmAbi,
...
mod solidity;
use solidity::{get_evm_contract_path, read_evm_u64_entry};
...
async fn test_evm_end_to_end_morpho_not_reentrant() -> Result<()> {
    ...
    let (evm_contract, _dir) = get_evm_contract_path("morpho_test_code/test/SimpleNonReentrantTest.sol")?;
